...
class patient_pdf_c extends CI_Controller {
    ...
    $data = $this->patient->view_transaction($id,$from,$to);
    $massage = $this->patient->count_massage($from,$to,$id);
    $clothes = $this->patient->count_clothes($from,$to,$id);
    $water = $this->patient->count_water($from,$to,$id);
    $cr= $this->patient->count_cr($from,$to,$id);
    $emergency = $this->patient->count_emergency($from,$to,$id);
    $patient = $this->patient->patient_name($id);
            ...
            <table>
                <tbody><br><br>
                      <tr style=\"font-size:11px; text-decoration: none; color: #000000; background-color:#878787; text-align: center;\">  <th style=\" width:15%;font-size:10px;\"><strong>Caretakers Name</strong></th>
                            <th style=\" width:6%;\"><strong>Need</strong></th>
                            ...
                            <th style=\" width:24%;\"><strong>Details</strong></th>
                ...
                ";
                    foreach($data as $d){
                        ...
                        $html2 .="<tr><td style=\"\"><h6>".$d->fname." ".$d->mname." ".$d->lname."</h6></td>
